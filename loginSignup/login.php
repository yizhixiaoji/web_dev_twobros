...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["customerId"]))){
        $customerId_err = "Please enter customerId.";
    } else{
    ...
    if(empty($customerId_err) && empty($password_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                // Check if customerId exists, if yes then verify password
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    // Bind result variables
                    mysqli_stmt_bind_result($stmt, $id, $customerId, $hashed_password);
                    if(mysqli_stmt_fetch($stmt)){
                        if(password_verify($password, $customerId)){
                    ...
                    // Display an error message if customerId doesn't exist
                    $customerId_err = "No account found with that customerId.";
                }
            } else{
